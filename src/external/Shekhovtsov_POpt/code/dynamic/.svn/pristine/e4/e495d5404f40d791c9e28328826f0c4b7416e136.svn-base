...
namespace dynamic{
	...
	*/
	...
	public:
		...
		bool can_allocate(size_t size_bytes, int align)const;
	public:
		stack_allocator();
		//stack_allocator()
	...
	*/
	...
	private://forbidden
		...
		void * protect_allocate(size_t size_bytes, int align);
	...
	public:
		void * allocate(size_t size_bytes, int aling = sizeof(size_t));
		size_t object_size(void * vP);
		void deallocate(void * vP);
		void* realloc(void * vP, size_t size_bytes);
		void error_allocate(big_size size_bytes, const char * caller);
		void check_integrity();
	};
...
namespace custom_new{ // this is not possible in the standard, works olny on MSVC, depricated
#ifdef CUSTOM_NEW
