...
namespace dynamic{
	...
	template <class type, int dims, class Allocator = std::allocator<type> > class num_array : public ivector<num_array<type, dims, Allocator >, dynamic::heap_buffer<type, Allocator> >{
	public:
		typedef dynamic::heap_buffer<type, Allocator> buffer;
		typedef ivector<num_array<type, dims, Allocator >, buffer > parent;
	...
	public:
		...
		int linindex(int i0, int i1, int i2 = 0, int i3 = 0, int i4 = 0)const{
			switch (dims){
			case 1: return i0;
			case 2: return i1*CC[0] + i0;
			case 3: return (i2*CC[1] + i1)*CC[0] + i0;
			case 4: return ((i3*CC[2] + i2)*CC[1] + i1)*CC[0] + i0;
			case 5: return (((i4*CC[3] + i3)*CC[2] + i2)*CC[1] + i1)*CC[0] + i0;
			};
			return -1;
		};
		//! compute multiindex
	...
	public:
		...
		const type & operator()(int i, int j, int k=0, int l=0, int m=0)const{
			return buffer::operator[](linindex(i, j, k, l, m));
		};
		type * begin(){ return buffer::begin(); };
		const type * begin()const{ return buffer::begin(); };
		type * begin(int i, int j, int k = 0, int l = 0, int m = 0){
			return begin() + linindex(i, j, k, l, m);
		};
		const type * begin(int i, int j, int k = 0, int l = 0, int m = 0)const{
			return begin() + linindex(i, j, k, l, m);
		};
		int stride(int i, int j, int k=0, int l=0, int m=0)const{
			return linindex(tindex(i,j,k,l,m));
		};
		int byte_stride(int i, int j, int k=0, int l=0, int m=0)const{
			return linindex(tindex(i,j,k,l,m))*sizeof(type);
		};
		...
		void set_zero(){
			memset(this->begin(),0,(this->end()-this->begin())*sizeof(type));
		};
		...
		*/
		...
		void cumsum(type(*foo)(type x)=&identity){
			if(dims==1){
				...
				//process first plane
				while(p<this->begin()+stride1){
					(*p) = foo(*p);
					++p;
				};
				while(p<this->begin()+stride2){
					(*p) = foo(*p);
					*p += *(p-stride1);
					++p;
				};
