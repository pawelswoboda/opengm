...
#else
#define DEFAULT_ALLOCATOR dynamic::array_allocator<type>
#endif
...
namespace dynamic{
	...
	*/
	...
	public:
		const type * begin()const{ return _beg; };
		type * begin(){return _beg;};
		const type * end()const{ return _end; };
		...
		bool owned()const{// is a non-empty owned
			return (_end > _beg && _cap > _beg);
		};
		bool is_ref()const{// is a non-empty reference
			return (_end > _beg && _cap == _beg);
		};
	...
	private:
		...
		void inline create_copy(type * beg, type *end, const type *it){
			while(beg<end){
	...
	public:
		...
		//! attach to some memory without allocating
		void set_ref(void * beg, int size){
			assert(empty() || is_ref());
			_beg = (type*)beg;
			_end = _beg + size;
			_cap = _beg; // ok this actually indicates nothing to destroy
		};

		fixed_array1(void * beg, int size){
			set_ref(beg, size);
		};
		...
		void operator = (const fixed_array1 & x){
			const int xs = x.size();
			if(&x==this)return;
			if (!x.allocated() && x._beg == x._end){ //x could be a reference too
				clear();
				return;
			};
			if (_end > _beg && _cap==_beg){// reference
				//only works if no resizing occurs
				if (xs != size()){
					throw debug_exception("refernce vector size does not match");
				};
				std::copy(x.begin(), x.begin() + xs, begin());   //copy size() elements by type::operator = 
				return;
			};
			// not a reference
			if(!allocated()){
