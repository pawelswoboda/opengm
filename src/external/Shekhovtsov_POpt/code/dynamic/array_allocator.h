...
namespace dynamic{
	...
	template<class T, int align = sizeof(size_t)> class array_allocator{
	public:
	...
	public:
		pointer allocate(size_type n, const void *hint=0){
			big_size size_bytes = sizeof(T)*(big_size)(n);
			if (size_bytes > std::numeric_limits<big_size>::max()){
				dynamic::memserver::error_allocate(size_bytes);
			};
			void * P = memserver::get_global()->allocate(size_t(size_bytes), align);
			return (T*)P;
		};
