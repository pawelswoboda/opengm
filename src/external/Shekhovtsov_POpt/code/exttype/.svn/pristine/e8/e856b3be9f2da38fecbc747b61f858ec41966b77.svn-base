...
namespace exttype{
	...
	*/
	...
	public:
		...
		tvector operator /(const field k)const{return cpy()/=k;};
		tvector & neg(){
			for(int i=0;i<this->count();i++)(*this)[i]=-(*this)[i];
			return *ths();
		};
		...
		std::pair<field, int> second_min()const{
			field th = min().first;
			array_entry<field> r(std::numeric_limits<field>::max(), 0);
			for (int i = 0; i<this->count(); i++){
				if ((*this)[i] > th && (*this)[i] < r.value){
					r.value = (*this)[i];
					r.arg = i;
				};
			};
