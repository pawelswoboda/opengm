...
class quiet_exception : public std::exception{
private:
	dstring smsg;
	void halt(){
		// do nothing
...
public:
	quiet_exception() :smsg("unspecified"){
		assert(false);
	};

	quiet_exception(const char * msg) :smsg(msg){
		halt();
	};
	quiet_exception(const dstring & msg) :smsg(msg){
		halt();
	};
	quiet_exception(const quiet_exception & x) :smsg(x.smsg){
		halt();
	};
	virtual ~quiet_exception() throw (){};
public:
	virtual const char *what() const throw(){
		return smsg.c_str();
	};
