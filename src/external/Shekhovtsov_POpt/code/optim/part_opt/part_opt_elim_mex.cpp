...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	*/
	...
	for(int e = 0;e<nE;++e){
		...
		int t = E(1,e);
		//
		int a_st = max_int; // a_st
		if(z(t)==y(t))a_st = f2(z(s),y(t),e) - f2(y(s),y(t),e);
		for(int j=0;j<K;++j){
			if(j==z(t) || X(j,t)==0)continue;
			a_st = std::min(a_st,( f2(z(s),j,e) - f2(y(s),j,e) ));
		};
		//if(a_st==max_int)a_st = f2(z(s),y(t),e) - f2(y(s),y(t),e);
		a0_st(e) = a_st;
		//
		int a_ts = max_int; // a_ts
		if(z(s)==y(s))a_ts = f2(y(s),z(t),e)-f2(y(s),y(t),e);
		for(int i=0;i<K;++i){
			if(i==z(s) || X(i,s)==0)continue;
			a_ts = std::min(a_ts,( f2(i,z(t),e)-f2(i,y(t),e) ));
		};
		//if(a_ts==max_int)a_ts = f2(y(s),z(t),e)-f2(y(s),y(t),e);
		a0_ts(e) = a_ts;
		//
