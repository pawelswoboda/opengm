   ...
   ,_fout(param.verbose_ ? fout : *OUT::nullstream::Instance())//(fout)
      ...
      InferenceTermination CombiLP_base<GM,ACC,LPREPARAMETRIZER,ILPSOLVER>::_PerformILPInference(GMManipulatorType& modelManipulator,std::vector<LabelType>* plabeling)
      {
         ...
         for (size_t modelIndex=0;modelIndex<modelManipulator.numberOfSubmodels();++modelIndex)
         {
            ...
            typedef hack::CanonicalReparametrizer<ReparametrizedGMType> CanonicalReparametrizerType;
            ...
            typedef LabelCollapse<typename LPReparametrizer<ReparametrizedGMType, AccumulationType>::ReparametrizedGMType, LPCplex<AuxMType, ACC> > ILPSolverType;

            typename ILPSolverType::Parameter param;
            param.proxy.integerConstraint_ = true;
            param.proxy.numberOfThreads_ = 4;

            ILPSolverType ilpSolver(m, param);
      ...
      InferenceTermination CombiLP_base<GM,ACC,LPREPARAMETRIZER,ILPSOLVER>::infer(MaskType& mask,const std::vector<LabelType>& lp_labeling,VISITORWRAPPER& vis,ValueType value_, ValueType bound_)
      {
    	  _value=value_;
    	  _bound=bound_;
#ifdef TRWS_DEBUG_OUTPUT
